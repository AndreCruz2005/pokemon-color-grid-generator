...
def get_pokemon_colors() -> dict:
    with open("src/pokemon_color_data.json", "r") as file:
        data = json.load(file)
        return data
    
def get_image(color, url):
    try:
        response = requests.get(url)
        img = Image.open(BytesIO(response.content)).convert("RGBA")
        if color not in images:
            images[color] = []
        images[color].append(img)
...
def create_image():
    ...
    for color, pokemon_list in get_pokemon_colors().items():
        for pokemon_url in pokemon_list:
            t = threading.Thread(target=get_image, args=(color, pokemon_url))
            t.start()
            threads.append(t)
    ...
    images = {key: images[key] for key in ["white", "red", "orange", "yellow", "green", "blue", "violet", "pink", "black"]}
